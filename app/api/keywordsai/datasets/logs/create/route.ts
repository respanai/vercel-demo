...
export async function POST(req: Request) {
  ...
  const { input, output, metadata, metrics } = body ?? {};
  const payload = {
    input: input ?? { message: "Demo dataset input" },
    output: output ?? { response: "Demo dataset output" },
    metadata: metadata ?? { custom_identifier: "dataset-demo" },
